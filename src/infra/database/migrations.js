...
class MigrationRunner {
  ...
  async getExecutedMigrations() {
    try {
      const migrations = await this.db("migrations_history")
  ...
  async recordMigration(name, status = "success", errorMessage = null) {
    try {
      const existing = await this.db("migrations_history")
        .where("name", name)
        .first();
      if (existing) {
        await this.db("migrations_history").where("name", name).update({
          status,
          error_message: errorMessage,
          executed_at: this.db.fn.now(),
        });
        return;
      }
      await this.db("migrations_history").insert({
        name,
        status,
        error_message: errorMessage,
      });
      ...
      console.warn(
        `Warning: Could not record migration history: ${error.message}`,
      );
    }
  }
  ...
  async migrationHistoryTable() {
    try {
      ...
      console.error(
        "Failed to initialize migrations history table:",
        error.message,
      );
    }
  }
    ...
    try {
      ...
      for (let i = migrationFiles.length - 1; i >= 0; i--) {
        ...
        try {
          ...
          await this.db("migrations_history")
            .where("name", migrationName)
            .update({
              status: "rolled_back",
              rolled_back_at: this.db.fn.now(),
            });
          console.log(`✅ Rolled back: ${migrationName}`);
        } catch (error) {
          console.error(
            `❌ Rollback failed: ${migrationName} - ${error.message}`,
          );
  ...
  async checkOrCreateMigration(migrationName) {
    try {
      ...
      const exists = files.some(
        (f) => path.basename(f, ".js") === migrationName,
      );
      ...
      if (exists) {
        ...
        fs.writeFileSync(
          filePath,
          this.messageText.migrationCreate.replaceAll(
            "{migrationName}",
            migrationName,
          ),
        );
  ...
  async getStatus() {
    try {
      ...
      const pending = migrationFiles
        ...
        .filter((m) => !executedMigrations.includes(m));
  ...
  async resetMigrations() {
    ...
    try {
      ...
      console.log(
        `✨ Migrations reset successfully! Ready for fresh migration.\n`,
      );
    } catch (error) {

...
class seederRunner {
  ...
  async getSeedFiles() {
    try {
      const files = fs.readdirSync(SEEDS_DIR).filter(file => file.endsWith(".js")).sort();
      return files;
    } catch (error) {
    ...
    try {
      ...
      for (const file of seedFiles) {
        ...
        try {
          ...
          if (!exists) {
            console.log(`⏭️  Skipped: ${seedName} (table '${seedTableName}' does not exist)`);
            continue;
          }
  ...
  async runSingleSeed(seedName) {
    ...
    try {
      ...
      if (!matchedFile) {
        ...
        seedFiles.forEach(f => { console.log(`  • ${path.basename(f, ".js")}`);});
        process.exit(1);
      }
  ...
  async checkOrCreateSeeder(seederName) {
    try {
      ...
      if (exists) {
        ...
        fs.writeFileSync(filePath, this.messageText.seederCreate.replaceAll("{seederName}", seederName));

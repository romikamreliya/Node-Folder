...
class demoController extends BaseController {
  ...
  async uploadFile(req, res) {
    const uploadPath = process.env.UPLOAD_PATH || 'public/upload';
    
    try {
      if (!req.file) {
        return this.response.send({ req, res, type: "BAD_REQUEST", message: "NO_FILE_PROVIDED" });
      }
      ...
      return this.response.send({ req, res, type: "SUCCESS", message: "FILE_UPLOADED", data: fileData });
    } catch (error) {
      throw new this.appError({ message: "FILE_UPLOAD_FAILED", type: "BAD_REQUEST" });
    }
  }
